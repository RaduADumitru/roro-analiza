...
Outputs `eda/plots/avg_sentence_length_judete_vs_raioane_<dataset>.png` with the dataset name in the title and file name.
"""
...
def plot_values(values: dict, out_path: Path, dataset_name: str):
    labels = list(values.keys())
    data = [values[k]['avg'] for k in labels]

    plt.figure(figsize=(6, 4))
    bars = plt.bar(labels, data, color=['#4c72b0', '#dd8452'])
    plt.title(f"Average words per sentence — judete vs raioane\n({dataset_name})")
...
def main():
    ...
    out_path = Path(__file__).resolve().parent / 'plots' / f'avg_sentence_length_judete_vs_raioane_{dataset_name}.png'
    plot_values(values, out_path, dataset_name)
