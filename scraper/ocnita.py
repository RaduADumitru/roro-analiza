...
BASE_URL = "https://ocnita.md"
OUTPUT_DIR = Path("data-cleaned/raioane/RepMoldova/Ocnita")
...
def get_article_urls_from_page(page_num: int):
    ...
    for article in soup.find_all("article"):
        h2 = article.find("h2", class_="post-box-title")
        link = h2.find("a") if h2 else None
        href = link.get("href") if link else None
        if not href:
            # Fallback: any <a> under the article
            fallback_link = article.find("a")
...
def extract_article_data(url: str):
    ...
    # Title: h1.name.post-title.entry-title, fallback to <title>
    title = ""
    h1 = soup.find("h1", class_="name post-title entry-title")
    if not h1:
        h1 = soup.find("h1", class_="entry-title") or soup.find("h1", class_="post-title")
    if h1:
    ...
    if not entry:
        entry = soup.find("div", class_="post-content") or soup.find("article", class_="post")
...
def main():
    ...
    for idx, url in enumerate(unique_urls):
        ...
        filename = f"Ocnita_Md_{success_count:04d}.json"
