...
BASE_URL = "https://primariasoldanesti.md"
OUTPUT_DIR = Path("data-cleaned/raioane/RepMoldova/Soldanesti")


def build_page_url(page_num: int) -> str:
    if page_num <= 1:
        return f"{BASE_URL}/articol/noutati/"
    return f"{BASE_URL}/articol/noutati/page/{page_num}/"
...
def get_article_urls_from_page(page_num: int):
    ...
    for article in soup.find_all("article", class_="wd-post"):
        h3 = article.find("h3", class_="wd-entities-title")
...
def extract_article_data(url: str):
    ...
    # Title: h1.wd-entities-title
    title = ""
    h1 = soup.find("h1", class_="wd-entities-title")
    if h1:
    ...
    # Remove unwanted classes and widgets (including download widgets, galleries, etc.)
    for cls in [
        "wp-block-embed",
        "wp-block-gallery",
        "meks-ess-share",
    ...
    # Remove figure tags (images/galleries)
    for tag_name in ["figure", "aside", "nav", "form"]:
...
def main():
    ...
    for idx, url in enumerate(unique_urls):
        ...
        filename = f"Primaria_Soldanesti_{success_count:04d}.json"
