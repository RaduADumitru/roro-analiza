"""
Web scraper for nisporeni.md website
...
# Base URLs
BASE_URL = "https://nisporeni.md"

# Output directory
OUTPUT_DIR = Path("data-cleaned/raioane/RepMoldova/Nisporeni")


def build_page_url(page_num: int) -> str:
    """Return the listing URL for a given page number."""
    if page_num <= 1:
        return f"{BASE_URL}/noutati-si-evenimente/"
    return f"{BASE_URL}/noutati-si-evenimente/page/{page_num}/"
...
def get_article_urls_from_page(page_num: int):
    ...
    # Articles on listing pages are in <article> tags with link in h3.title > a
...
def extract_article_data(url: str):
    """Extract title and content from a nisporeni.md article page."""
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
    }
    ...
    # Title extraction: get from h1.entry-title
    title = ""
    ...
    # Content extraction: look for div with class "content-inner"
    ...
    if content_container:
        ...
        # Remove download links and file buttons
        for download_link in content_container.find_all("a", class_="wp-block-file__button"):
            download_link.decompose()
        
        for download_link in content_container.find_all("a", attrs={"download": True}):
            download_link.decompose()

        # Remove social sharing widgets and download widgets
        ...
        for figure in content_container.find_all("figure"):
            figure.decompose()
...
def main():
    ...
    for idx, url in enumerate(unique_urls):
        ...
        filename = f"Nisporeni_Md_{success_count:04d}.json"
