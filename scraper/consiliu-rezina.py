...
BASE_URL = "https://consiliu.rezina.md"
OUTPUT_DIR = Path("data-cleaned/raioane/RepMoldova/Rezina")


def build_page_url(page_num: int) -> str:
    return f"{BASE_URL}/stiri-si-evenimente/blog/page/{page_num}/"
...
def get_article_urls_from_page(page_num: int):
    ...
    # Find articles in article.blog-article with a.blog-article__thumbnail link
    for article in soup.find_all("article", class_="blog-article"):
        link = article.find("a", class_="blog-article__thumbnail")
        if link:
            href = link.get("href")
            if href:
...
def extract_article_data(url: str):
    ...
    if not title:
        ...
        if title_tag:
            ...
            title = title_text.split(" - Consiliul Raional Rezina")[0].strip() if " - Consiliul Raional Rezina" in title_text else title_text

    # Content: div.single-page__content
    entry = soup.find("div", class_="single-page__content")
...
def main():
    ...
    for idx, url in enumerate(unique_urls):
        ...
        filename = f"Consiliu_Rezina_{success_count:04d}.json"
