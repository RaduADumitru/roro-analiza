"""
Scraper for primariaedinet.md news section.
Iterates listing pages, collects article URLs, extracts title and content, and saves JSON files.
"""
...
BASE_URL = "https://primariaedinet.md"
BASE_LISTING_URL = f"{BASE_URL}/sectiune/stiri"
OUTPUT_DIR = Path("data-cleaned/raioane/RepMoldova/Edinet")


def build_page_url(page_num: int) -> str:
	if page_num <= 1:
		return f"{BASE_LISTING_URL}/"
...
def get_article_urls_from_page(page_num: int):
	...
	for article in soup.find_all("article"):
		header = article.find("h3", class_="entry-title")
...
def extract_article_data(url: str):
	...
	if not title:
		...
		if meta_title:
			raw_title = meta_title.get_text(strip=True)
...
def main():
	...
	for idx, url in enumerate(unique_urls):
		...
		filename = f"PrimariaEdinet_{success_count:04d}.json"
